class LeadIntelligence {
    ...
    loadLocationScores() {
        return {
            new_york: { economy: 95, digital: 95, competition: 90 },
            los_angeles: { economy: 90, digital: 90, competition: 85 },
            chicago: { economy: 85, digital: 85, competition: 80 },
            miami: { economy: 85, digital: 80, competition: 75 },
            austin: { economy: 90, digital: 95, competition: 85 },
            default: { economy: 70, digital: 70, competition: 60 }
        };
    }
    ...
    scoreBusinessQuality(lead) {
        ...
        if (lead.address) {
            ...
            if (address.match(/\d+\s+\w+/)) score += 5; // Has number and street name
            if (address.includes('suite') || address.includes('floor') || address.includes('bldg')) score += 5;
    ...
    scoreLocation(lead) {
        ...
        // Major US Hubs
        if (address.includes('new york') || address.includes('ny') || address.includes('manhattan')) return this.locationScores.new_york.economy;
        if (address.includes('los angeles') || address.includes('california') || address.includes('ca')) return this.locationScores.los_angeles.economy;
        if (address.includes('chicago') || address.includes('illinois') || address.includes('il')) return this.locationScores.chicago.economy;
        if (address.includes('miami') || address.includes('florida') || address.includes('fl')) return this.locationScores.miami.economy;
        if (address.includes('austin') || address.includes('texas') || address.includes('tx')) return this.locationScores.austin.economy;
    ...
    scoreContactability(lead) {
        ...
        if (lead.phone) {
            score += 50;
            // Mobile numbers are more contactable
            if (lead.phone.includes('+1') || lead.phone.replace(/\D/g, '').length === 10) score += 20;
        }
